...
We based our two tasks on choosing a university and picking a restaurant for a large social event, because in PS.1 the majority of people stated that they had used multiple websites for these tasks. We used different two different variables for both tasks, where the restaurant was located (Glasgow and Cambridge) and which subject people chose (sport science and history). For the restaurant task, we chose the top restaurants in Glasgow and Cambridge from Tripadvisor and Yelp as the candidates. We chose these cities because they are not geographically close to the participants, to minimise bias). For the university condition we selected two university courses at random, and selected the top six universities for each subject. We selected a shortlist of six because it is typical for users to narrow down their options because analysing the final details of their choice.
