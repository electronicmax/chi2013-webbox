...
In order to feel familiar to new users, we borrowed the overall  ``look and feel'' from MacOS Finder-style file managers.  In the DataPalette workspace, shown in Figure \ref{fig:workspace}, the user can arbitrarily drag and drop entities from data sources (on the left) into windows that they create, resize, arrange and label. (Although the design could be generalised to supporting nested groups, our prototype only supported a single level of grouping.)
...
Just as the wise Italian in Voltaire's poem \emph{La Begueule} predicted,``the best is the enemy of the good'', we believe that the quest for complete approaches to data integration have perhaps caused more scruffy, ``ad-hoc'' approaches to be overlooked.  Based upon the results of our investigation, we believe that there are a number of advantages to user-driven data reconciliation over automatic to bespoke ones; specifically, this enables the user to combine data as part of his or her sensemaking process, using subjective judgement in the process
