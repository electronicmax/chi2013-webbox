...
\begin{abstract}
While people naturally draw from diverse information sources in the course of routine decision-making, complementary support for managing diverse, heterogeneous data in digital Personal Information Management (PIM) tools remains poor.  In this paper, we present an exploratory initial investigation of user-driven, ad-hoc data integration approaches for PIM, towards enabling greater use of the emerging ecosystems of structured data available on the Web.  We conducted an exploratory, sequential, mixed-method study, starting with two pre-studies focusing on the data integration needs and challenges, respectively, of effectively combining data from diverse Web-based structured data sources.  These observations led to the design of DataPalette, an interface for ``light-touch'' data integration that introduced drag-and-drop and path-selection mechanisms for managing collections of terminologically and structurally- heterogeneous data items.  Our lab study of DataPalette with actual data obtained from a selection of Web sites  showed that users understood the interaction mechanisms introduced and the problems they were designed to address, and that DesignPalette allowed them to make more carefully informed decisions and draw from more data sources while performing multidimensional subjective choice-based tasks.
...
\hline
4) Restaurant they would book for 12 friends in Glasgow & Restaurants :1) Black Sheep Bistro, 2) Charcoals, 3) Number 16 Family Restaurant, 4) Fanny Trollope's Cafe, 5) Two Fat Ladies At The Buttery, and 6) Sapori D'Italia. & www.yelp.co.uk, www.tripadvisor.co.uk, http://ratings.food.gov.uk/, facebook and google plus.\\
\hline
\end{tabular}
\end{center}
...
Twenty-six (26) participants responded, from which we selected the first 20.  Ten participants were female.  All of the participants spoke English, but 12 of them were non-native English speakers.  Seven were non-students, comprising University staff and alumni; out of the remaining 13, eight studied computer science and 5 studied other subjects.   On average, each study took 40 minutes to complete.  The information in Table \ref{tbl:data} summarises the metrics that were captured of the trials for each participant. We analyse this data next.
...
A key design decision we made at the outset was to target the most common, simpler forms of heterogeneity we observed in PS.2.  Given the use of the interface mechanisms we observed, we wish to next broaden the scope of capabilities of DP so that it can become more useful in real settings.  Among the integration capabilities we are considering are, first, field combination and splitting (by delimiters), in order to address the kinds of structural heterogeneity observed in several cases of PS.2, along with a framework for converting and reconciling units of measure, which would address several of the modelling issues.  Extending the Do What I Mean capabilities of the histogram and bar-charting tools to be unit-aware would be a tremendous asset, we believe, for users who want to quickly compare things without paying close attention to the units used.  A core problem to realising this, however, is that few data sources (besides the Weather category) explicitly annotated values with units used.  

\subsection{Study Limitations}

Our study had several limitations.  First, our study consisted of 20 participants which meant that we could not show that some of our data was statistically significant.  Second, we gave the participants an artificially limited amount of time (10 minutes) to make their decisions. Given additional time, the participants choices may have changed. Third, the data sources were fixed, because we were not testing the users' capabilities to search and retrieve data from the web. Given unlimited access to data sources online, users would be able to compare different aspects, which may also alter their choices.
