...
\numberofauthors{1}
 \author{
   \alignauthor Max Van Kleek, Daniel A. Smith, Heather S. Packer, Jim Skinner, Nigel R. Shadbolt \\
     \affaddr{School of Electronics and Computer Science}\\
...
A common goal for the release of such data has been to provide end-user citizens an ability to make more informed decisions pertaining to their health, wealth, and well-being \cite{Shadbolt:2006:SWR:1155313.1155373}.  Thus far, however, this data has been predominately been consumed by app developers, journalists and other ``data specialists'' in one-off data analyses, apps and investigations, remaining still far from influencing the activities of ordinary end-user citizens. A core reason for this, we posit, is a lack of suitable tools for accessing and interrogating these heterogeneous data sets, and a means by which these disparate information sources can be effectively brought together and cross-referenced.  For example, most personal information management (PIM) tools only provide facilities for managing small, pre-fixed sets of data types (such as digital calendaring tools and to-do list managers) or provide little or no support for structured data at all, such as text editors and sketching/drawing tools \cite{infoscraps}.   
...
In this paper, we present an investigation into ways that PIM tools might be extended to allow data to be brought together in an ad-hoc fashion from arbitrary structured data sources, and interrogated, so as to enable end-user citizens to effectively harness knowledge from the vast, emerging ecosystems of data.  We used a three-stage sequential exploratory mixed-method approach in this investigation: first, we performed a semi-structured interview pre-study to understand the types of tasks people performed using multiple information sources, and the processes that they relied upon to perform such tasks.  Our results suggested that people routinely drew upon multiple and diverse sources for reasons including coverage, reliability, and to gain multiple perspectives or subjective assessments.  
...
Second, we conducted a structural analysis of data records from personal data sources to identify the most common kinds of problems likely to arise from integrating data from these types of sources.  We examined data sources in six common domains: contacts, events, music, shopping, social networking and weather. Our analysis found that while terminological heterogeneity was the most common issue for unifying simpler data records (such as address book contacts and social network profiles), structural differences were exhibit among more complex schemas, such as online retailers' product catalogues.
...
The user experience benefits of conquering such data integration challenges have been demonstrated in PIM prototype systems with integrative data models.  For example, the uniform query capabilities provided by SEMEX~\cite{semex} and \emph{Stuff I've Seen}\cite{Dumais:2003:SIS:860435.860451}, showed the speed, efficiency and re-findability gained when a user is able to quickly trace and cross-reference information about the people, places and things mentioned throughout his or her files, folders, e-mail, and other information repositories.  Similarly, Haystack \cite{haystack} demonstrated the reduced effort and increased efficiency of being able to view and re-use information easily in multiple task contexts without fragmentation or loss of consistency when a single, consolidated data model was used across applications. 

Therefore, despite the multifaceted challenges of data integration, various fields have sought methods to resolve them. The database integration and Semantic Web \cite{Shadbolt:2006:SWR:1155313.1155373} fields, for example, have pursued automatic, recently machine-learning approaches to data integration, at either the level of \emph{ontology/schema-level matching}~\cite{euzenat2004api,doan2003learning}, or \emph{instance matching}~\cite{suchanek2011paris,castano2006matching}.   In contrast, the end-user programming community has pursued user-driven approaches, in which the user orchestrates the process of reconciliation at various levels of specificity.  Systems that use this approach include ``mash-up makers'' (such as Mashmaker, \cite{intel_mashmaker}, Marmite \cite{Lin:2009:EPM:1502650.1502667}, and Vegemite \cite{Wong:2007:MMM:1240624.1240842}) and visual programming languages such as Yahoo Pipes \cite{citeulike:8887891}.

Our approach is most similar to systems that, like the end-user programming approaches, allow users to easily perform data reconciliation using concrete instances as examples, but that avoid the need for any sort of ``programming'' by the user.  Prior systems in this space include ``Cards, Relations and Templates''\cite{Dontcheva:2007:RCS:1294211.1294224} and Potluck \cite{citeulike:3875264} which both allow users to reconcile instances from different schemas into a uniform representation using drag-and-drop gestures to specify relations among fields.  Our system, \emph{DataPalette} extends this approach to a data workbench environment which aims to allow people to easily consolidate and compare instances by their properties.
...
% \subsubsection{Task preparation} In order to prepare the data for our tasks, we identified websites that people commonly used for selecting universities and restaurants (see Table \ref{tab:studyfactors}).  Once we had compiled the list of websites, we select the top most used six for each task. 
...
%% the abililty to  In our investigation we found that complete approaches to data integration
